...
namespace ProtankiTool.Services
{
    public class UpdaterService
    {
        private const string Owner = "Gabriel250903";
        private const string Repo = "ProtankiTool";
        ...
        public async Task<(bool, string?)> DownloadAndInstallUpdateAsync(Action<long, long> progressChanged)
        {
            ...
            using (JsonDocument jsonDoc = JsonDocument.Parse(response))
            {
                ...
                if (latestRelease != null)
                {
                    ...
                    {
                        if (asset.GetProperty("name").GetString() == "ProtankiTool.Installer.msi")
                        {
                            ...
                            string tempPath = Path.Combine(targetDirectory, "ProtankiTool.Installer.msi");
        ...
        private async Task<bool> UninstallCurrentVersionAsync()
        {
            ...
            string productDisplayName = "ProtankiTool";
